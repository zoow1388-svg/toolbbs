...
1. 从 GitHub Release 下载 `codex-project-orchestrator-v1.9.1.zip` 和对应的 `.sha256` 文件。
2. 在文件所在目录验证 SHA-256：

```powershell
Get-FileHash -Algorithm SHA256 '.\codex-project-orchestrator-v1.9.1.zip'
```
...
升级到 v1.9.1 后，已有项目状态会在下一次受控写入时补充 Git 配置和任务字段。空的工作树配置会自动采用项目相邻目录；已有非空配置保持不变。旧的已完成记录不会被伪造为新证据。升级后先在非生产项目执行一次工作树、交接、暂存、提交、测试和审查流程，再决定是否用于正式项目。
