...
4. Create `.codex-orchestrator/workflow.json` and `tasks.json` only after the user authorizes state-file creation.
5. Build a dependency graph. Assign exclusive file ownership to every modifying task.
...
10. Validate normalized state with the JSON Schema and `scripts/validate-workflow.ps1` before advancing a gate.
