...
8. Configure the controller and use `prepare-dispatch` to create an immutable envelope. Before any `send_message_to_thread` call, run `begin-external-action` with the current controller epoch and use only its returned payload. Preserve the raw tool receipt, run `complete-external-action`, then apply that completed action with `record-sent` or `record-callback-ack` and the same external action ID. Never bypass the journal.
...
10. Require the worker to send the dispatch callback with `send_message_to_thread` before ending its completion turn. Save the incoming message, normalize it with `import-callback-receipt.ps1`, and record it with `record-callback`. Journal the planned `callback_ack` as an external action before sending it, then preserve and apply its receipt. Treat repeated callback event IDs as idempotent; reject mismatched task, dispatch, source, target, or host identities.
