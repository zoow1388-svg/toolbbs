[CmdletBinding()]
param(
    [Parameter(Mandatory=$true)][ValidateSet('initialize','register','transition','prepare-dispatch','record-sent','record-observation','record-ack','record-result','reconcile','show','audit')][string]$Action,
    ...
    [string]$DispatchId,[string]$MessageId,[string]$ResultMessageId,[string]$Cursor,[string]$ReceiptPath,
    [string]$ObservedProjectPath,[string]$ObservedStatus,[string]$ObservationPath
)
