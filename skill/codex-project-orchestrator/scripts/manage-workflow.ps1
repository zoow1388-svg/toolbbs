...
switch ($Action) {
    ...
    'resolve-action' { if(-not $ExecutionId -or -not $EvidencePath -or -not $Reason -or -not $Resolution){throw 'ExecutionId, EvidencePath, Reason, and Resolution are required.'};Resolve-WorkflowActionExecution -ProjectPath $ProjectPath -ExecutionId $ExecutionId -Resolution $Resolution -EvidencePath $EvidencePath -Reason $Reason|ConvertTo-Json -Depth 20 }
    'authorize-action-retry' { if(-not $ExecutionId -or -not $EvidencePath -or -not $Reason){throw 'ExecutionId, EvidencePath, and Reason are required.'};Authorize-WorkflowActionRetry -ProjectPath $ProjectPath -ExecutionId $ExecutionId -EvidencePath $EvidencePath -Reason $Reason|ConvertTo-Json -Depth 20 }
    ...
    'controlled-git' { if(-not $GitActionId -or -not $GitRequestPath -or -not $GitReceiptPath){throw 'GitActionId, GitRequestPath, and GitReceiptPath are required.'};$gitAction=@(Get-GitActionState -ProjectPath $ProjectPath|Where-Object{$_.git_action_id-eq$GitActionId});if($gitAction.Count-ne1-or$gitAction[0].status-ne'prepared'){throw 'Matching prepared Git transaction not found.'};if([IO.Path]::GetFullPath($GitRequestPath)-ne$gitAction[0].request_path){throw 'Git request path does not match transaction.'};& (Join-Path $PSScriptRoot 'controlled-git.ps1') -RequestPath $GitRequestPath -ReceiptPath $GitReceiptPath -ExpectedRequestSha256 $gitAction[0].request_sha256 }
