...
function Add-Defaults {
    ...
    if ([int]$Workflow.state_version -lt 15) { $Workflow.state_version = 15 }
}
...
function Get-LiveWorktreeIdentity {
    ...
    $branch=(& git -C $resolved symbolic-ref --quiet --short HEAD 2>$null);$detached=$LASTEXITCODE -ne 0
    $head=(& git -C $resolved rev-parse HEAD).Trim().ToLowerInvariant()
    if($LASTEXITCODE -ne 0){throw 'Unable to resolve worktree HEAD.'}
    [pscustomobject][ordered]@{repository_root=$roots[0];worktree_path=$resolved;branch_name=$(if($detached){$null}else{$branch.Trim()});head_revision=$head;is_detached=$detached;is_dirty=(@(& git -C $resolved status --porcelain=v1).Count -gt 0)}
}
...
function Set-WorkflowTaskWorktree {
    ...
    Invoke-WithStateLock $state {
        ...
        $workflow=Read-StateJson $workflowPath;$tasks=@(Read-StateJson $tasksPath|ForEach-Object{$_});Add-Defaults $workflow $tasks
        $task=@($tasks|Where-Object{$_.task_id -eq $TaskId});if($task.Count -ne 1){throw "Task not found: $TaskId"};$task=$task[0]
        if($task.role -notin @('developer','tester','reviewer')){throw 'Only developer, tester, or reviewer tasks can bind a worktree.'}
        ...
        foreach($name in @('mode','repository_root','worktree_path','branch_name','head_revision','is_detached','is_dirty','inspected_at')){if($binding.PSObject.Properties.Match($name).Count -eq 0){throw "Worktree binding missing field: $name"}}
        $expectedMode=$(if($task.role -eq 'developer'){'developer'}else{'verification'})
        if($binding.mode -ne $expectedMode){throw 'Worktree binding mode does not match task role.'}
        if($binding.is_dirty -or ($task.role -eq 'developer' -and $binding.is_detached)){throw 'Worktree mode, branch, or cleanliness is invalid for the task role.'}
        ...
        if($live.repository_root -ne [IO.Path]::GetFullPath($workflow.project_path) -or $live.branch_name -ne $binding.branch_name -or [bool]$live.is_detached -ne [bool]$binding.is_detached -or $live.head_revision -ne $binding.head_revision -or $live.is_dirty){throw "Worktree binding does not match current Git state: repository=$($live.repository_root -eq [IO.Path]::GetFullPath($workflow.project_path)); branch=$($live.branch_name -eq $binding.branch_name); detached=$([bool]$live.is_detached -eq [bool]$binding.is_detached); head=$($live.head_revision -eq $binding.head_revision); clean=$(-not $live.is_dirty)."}
        if($task.base_revision -ne $live.head_revision){throw 'Worktree HEAD does not match task base revision.'}
        $active=@($tasks|Where-Object{$_.task_id -ne $TaskId -and $_.role -in @('developer','tester','reviewer') -and $_.status -notin $script:TerminalStates})
        if(@($active|Where-Object{![string]::IsNullOrWhiteSpace($_.worktree_path) -and [IO.Path]::GetFullPath($_.worktree_path) -eq $live.worktree_path}).Count){throw 'Worktree path is already bound to an active task.'}
        if(-not [string]::IsNullOrWhiteSpace($live.branch_name) -and @($active|Where-Object{![string]::IsNullOrWhiteSpace($_.branch_name) -and $_.branch_name -eq $live.branch_name}).Count){throw 'Branch is already bound to an active task.'}
        ...
        $task.worktree_mode=$binding.mode;$task.worktree_binding_path=[IO.Path]::GetFullPath($target);$task.worktree_binding_sha256=(Get-FileHash $target -Algorithm SHA256).Hash.ToLowerInvariant();$task.worktree_path=$live.worktree_path;$task.branch_name=$live.branch_name;$task.worktree_head_revision=$live.head_revision;$task.worktree_is_detached=$live.is_detached;$task.worktree_bound_at=Get-UtcTimestamp;$task.project_path=$live.worktree_path;$task.updated_at=Get-UtcTimestamp
...
function New-WorkflowDispatch {
    ...
    Invoke-WithStateLock $state {
        ...
        $worktree=$null
        if($task.role -in @('developer','tester','reviewer') -and $task.base_revision -notin @('unborn','unknown','unavailable')){
            if([string]::IsNullOrWhiteSpace($task.worktree_path)){throw 'Commit-based task requires a verified worktree binding before dispatch.'}
            ...
            if($live.branch_name -ne $task.branch_name -or $live.is_detached -ne $task.worktree_is_detached -or $live.head_revision -ne $task.base_revision -or $live.is_dirty){throw 'Worktree branch, HEAD, or cleanliness changed before dispatch.'}
            $worktree=[ordered]@{mode=$task.worktree_mode;path=$task.worktree_path;branch_name=$task.branch_name;head_revision=$task.worktree_head_revision;is_detached=$task.worktree_is_detached;binding_path=$task.worktree_binding_path;binding_sha256=$task.worktree_binding_sha256}
        }
...
function Test-WorkflowStateIntegrity {
    ...
    $activeWorkspaces=@($tasks|Where-Object{$_.role -in @('developer','tester','reviewer') -and $_.status -notin $script:TerminalStates -and -not [string]::IsNullOrWhiteSpace($_.worktree_path)})
    if(@($activeWorkspaces|Group-Object worktree_path|Where-Object Count -gt 1).Count){throw 'Duplicate active task worktree path detected.'}
    if(@($activeWorkspaces|Where-Object{-not [string]::IsNullOrWhiteSpace($_.branch_name)}|Group-Object branch_name|Where-Object Count -gt 1).Count){throw 'Duplicate active task branch detected.'}
