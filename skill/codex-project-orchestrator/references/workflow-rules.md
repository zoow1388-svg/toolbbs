...
5. 使用真实提交作为基线的开发、测试和审查任务必须绑定独立且干净的 Git Worktree；开发使用分支模式，测试和审查使用验证模式并可绑定 detached HEAD。活动任务不得共享工作树路径或非空分支。
...
- 创建工作树、暂存、提交、合并分别使用不可变请求和回执，且每次都要求 `git-approved` 授权。
- 分支必须使用 `codex/` 前缀；工作树、分支已存在，仓库不干净或基线变化时立即停止。
- 暂存和提交只能涉及 `allowed_files`，不能隐式纳入范围外文件。
- 合并前必须同时具备测试和审查可信回执，并且两者检查的修订必须等于源分支当前提交。
- 合并冲突只报告，不自动解决；默认不推送、不强制合并、不删除分支或工作树。
- 所有 Git 操作必须先写入 `git-actions.json`。只有 `prepared` 事务能进入执行计划；`completed` 不重复执行，`failed` 只进入人工核查。
- 总控中断时不得仅凭租约或缺少回执判断操作未执行，必须核对 Git 仓库、请求哈希和回执后再决定完成、失败或取消。
