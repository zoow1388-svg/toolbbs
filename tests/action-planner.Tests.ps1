...
Describe 'workflow action planner' {
    ...
    It 'reports workflow completion only after trusted verification' {
        ...
        (New-Plan $project (Join-Path $project 'sent-plan.json')).actions[0].type|Should Be 'wait_tasks'
        ...
        $taskPath=Join-Path $project '.codex-orchestrator\tasks.json';$tasks=@(Get-Content $taskPath -Raw -Encoding UTF8|ConvertFrom-Json|ForEach-Object{$_});$tasks[0].latest_turn_id='turn-1';$tasks[0].latest_item_id='result-1';[IO.File]::WriteAllText($taskPath,($tasks|ConvertTo-Json -Depth 20),[Text.UTF8Encoding]::new($false))
