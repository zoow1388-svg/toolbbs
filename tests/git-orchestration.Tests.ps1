...
Describe 'v1.9 Git lifecycle orchestration' {
    ...
    It 'builds and completes a merge only from trusted gates on the same commit' {
        $script:autoRoot="D:\toolbbs-test-worktrees-$([guid]::NewGuid().ToString('N'))";&$manager -Action configure-git -ProjectPath $project -WorktreeRoot $script:autoRoot -TargetBranch main|Out-Null
