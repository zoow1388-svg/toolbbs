...
安全门禁拒绝磁盘根目录、项目目录本身以及项目内部目录，避免污染主仓库。真正创建工作树时才建立缺失的父目录；目标已存在、仓库脏、分支冲突或基线变化时仍立即停止。

## v1.9 Git 生命周期自动联动

v1.9 将任务状态和受控 Git 操作串成一条可恢复链路：总控按任务 ID 自动生成 `codex/` 分支和隔离工作树请求；开发窗口只修改授权文件并回传 `development-handoff.json`，不自行暂存或提交；总控核对实际修改后进入 `awaiting_commit`，依次生成暂存和中文 Conventional Commit 请求。
...
`git-actions.json` 纳入动作计划快照哈希，因此事务状态变化会让旧计划立即失效。中断后处于 `prepared` 的操作视为可能已经执行，必须核对仓库和回执，不能自动重试。远程推送、强制操作、冲突处理和清理仍不在自动范围内。
